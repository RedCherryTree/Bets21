		...
		try {
		   ...
		   if (month==12) { month=0; year+=1;}  
			...
			if (Locale.getDefault().equals(new Locale("es"))) {
				...
				q1.addQuote("Atletico", 1.5);
				q1.addQuote("Athletic", 1.4);
				q2=ev1.addQuestion("¿Quien metera el primer gol?",2);
				q2.addQuote("Atletico", 1.5);
				q2.addQuote("Athletic", 1.4);
				q3=ev11.addQuestion("¿Quien ganara el partido?",1);
				q3.addQuote("Atletico", 1.5);
				q3.addQuote("Athletic", 1.4);
				q4=ev11.addQuestion("¿Cuantos goles se marcaran?",2);
				q4.addQuote("1", 1.5);
				q4.addQuote("2", 1.4);
				q4.addQuote("3", 1.6);
				q5=ev17.addQuestion("¿Quien ganara el partido?",1);
				q5.addQuote("Malaga", 1.5);
				q5.addQuote("Valencia", 1.4);
    ...
    public void depositMoney(String user, double money, String paymentOpton, String paymentMethod) {
 	...
 	public Event deleteEvent(Integer eventNumber, Date eventDate, String reasonToRefund)throws EventDontExist{
		...
		if(ev==null) {
			...
			TypedQuery<MultipleQuoteBet> mulquery = db.createQuery("SELECT mqb FROM MultipleQuoteBet mqb WHERE mqb.hasEnded=="+false,MultipleQuoteBet.class);
			...
			for(MultipleQuoteBet mulBet: refund) {
				if(mulBet.isInEvent(ev)) {
					...
					List<Quote> betQuotes = mulBet.getBetQuotes();
						mulBet.setHasEnded(true);
						
						Transaction transaction=us.refundMultiple(mulBet, reasonToRefund);
